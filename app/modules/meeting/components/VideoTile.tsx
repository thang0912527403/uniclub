...
export const VideoTile: React.FC<VideoTileProps> = ({ 
  ...
  const hasVideo = stream && isVideoEnabled && stream.getVideoTracks().length > 0;
    ...
    >
      ...
      {isScreenSharing && (
        <div className="absolute top-3 left-3 flex items-center gap-2 bg-green-500/80 backdrop-blur-md px-2 py-1 rounded-lg border border-green-400/50">
          ...
          {!isThumbnail && <span className="text-white text-xs font-medium">Đang chia sẻ</span>}
        </div>
      )}
      ...
      <div className={`absolute inset-0 rounded-2xl ring-2 transition-all duration-300 pointer-events-none ${isSpotlight ? 'ring-orange-500/50' : 'ring-transparent group-hover:ring-orange-500/30'}`}></div>
    </div>
  );
};
