...
function LevelBadge({ level }: { level: number }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold border transition-all hover:scale-105 ${colors} uppercase tracking-wider`}
    >
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <span
              className={`font-semibold text-sm truncate ${isSelected ? "text-blue-700 dark:text-blue-300" : "text-gray-900 dark:text-white"}`}
            >
              {role.roleName}
            </span>
      ...
      {role.policies && role.policies.length > 0 && (
        <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
          ...
          <div className="flex flex-wrap gap-1.5">
            {(isExpanded ? role.policies : role.policies.slice(0, 4)).map(
              (p) => (
                <span
                  key={p.id}
                  className="px-2 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 rounded-md text-[9px] font-bold border border-gray-200 dark:border-gray-600/50 animate-in fade-in slide-in-from-top-1 duration-300"
                >
                  {p.title}
                </span>
              ),
...
function MemberAvatar({
  ...
  return (
    <div
      className={`w-16 h-16 ${color} rounded-2xl flex items-center justify-center text-white font-bold text-xl`}
    >
      {initials}
    </div>
  );
...
export default function ClubMemberRoleModule() {
  ...
  const { clubId: clubIdParam, memberId: memberIdParam } = useParams<{
    clubId: string;
    memberId: string;
  }>();
  ...
  const { data: roles, isLoading: isRolesLoading } =
    useGetClubStructureRolesQuery(clubId, { skip: !clubId });
  ...
  const effectiveSelected =
    selectedRoleIds === undefined ? currentRoles : selectedRoleIds;
  ...
  return (
    <div className="min-h-screen">
      <SettingButton />
      <Sidebar
        currentPath={clubIdParam ? `/clubs/${clubId}/members` : "/club/members"}
      ...
      >
        <div className="relative z-10">
          ...
          <button
            onClick={() =>
              navigate(
                clubIdParam ? `/clubs/${clubId}/members` : "/club/members",
              )
            }
              ...
              <div className="bg-white dark:bg-gray-800 rounded-[1.5rem] p-8 mb-8 border border-gray-200 dark:border-gray-700 shadow-sm relative overflow-hidden group">
                ...
                <div className="flex items-center gap-6 relative z-10">
                  ...
                  <div className="flex flex-col items-end gap-3">
                        ...
                        return (
                          <div className="flex flex-wrap justify-end gap-1.5 max-w-[320px]">
                            {(showAllRoles
                              ...
                              <span
                                key={i}
                                className="inline-flex items-center gap-2 px-3.5 py-2 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-xl text-[11px] font-bold whitespace-nowrap animate-in fade-in zoom-in duration-500 border border-blue-100 dark:border-blue-800 shadow-sm shadow-blue-500/5"
                              >
                    ...
                    <span
                      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black tracking-widest uppercase
                                        ${
                    ...
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full animate-pulse ${member.status?.toUpperCase() === "ACTIVE" ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "bg-gray-400"}`}
                      />
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400 dark:text-gray-500 font-black uppercase tracking-[0.2em] mt-0.5 opacity-80">
                        {isEditing
                          ? `Hệ thống: ${roles?.length ?? 0} Roles`
                          : `Thành viên: ${currentRoles.length} Roles`}
                      </p>
                    </div>
                  </div>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400 dark:text-gray-500 font-medium italic">
                          Thành viên sẽ không còn bất kỳ quyền hạn nào
                        </p>
                      </div>
                      <div
                        className={`ml-auto w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all duration-300
                      ...
                      >
                        {effectiveSelected.length === 0 && (
                          <i className="fas fa-check text-white dark:text-gray-900 text-[10px]" />
                        )}
                      </div>
                    </button>
          ...
          {!isLoading && !member && (
            <div className="max-w-md mx-auto mt-20 text-center">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">
                Thành viên không tồn tại hoặc đã bị xóa.
              </p>
              <button
                onClick={() =>
                  navigate(
