...
function MemberAvatar({
  ...
  member:
    | ClubMember
    | { clubMemberId: number; fullName: string; avatar: string | null };
  size?: "sm" | "md";
}) {
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
    >
      <span
    ...
    >
      ...
      >
        <div className="flex items-center gap-4 mb-4">
          <div
            className={`w-12 h-12 rounded-2xl flex items-center justify-center ${isActive ? "bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400" : "bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400"}`}
          >
            <i
              className={`fas ${isActive ? "fa-user-slash" : "fa-user-check"} text-xl`}
            />
          </div>
          <div>
...
function RoleCell({ member, clubId }: { member: ClubMember; clubId: number }) {
  ...
  return (
    <button
      onClick={() => navigate(`/club/members/${member.clubMemberId}/roles`)}
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      show({
        type: "success",
        title: "Thành công!",
        message: `Đã thêm ${member.fullName} vào phòng ban.`,
        duration: 3000,
      });
      onClose();
    } catch (err: any) {
      show({
        type: "error",
        title: "Thất bại",
        message: err?.data?.message ?? "Không thể thêm vào phòng ban.",
        duration: 4000,
      });
    }
  };
          ...
          {isLoading ? (
            ...
            <div>
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
                Chọn phòng ban muốn thêm thành viên vào:
              </p>
              <div className="space-y-2">
                {departments.map((dept: any, i: number) => {
                  const deptId = dept.DepartmentId ?? dept.departmentId;
                  const deptName =
                    dept.Name ?? dept.name ?? dept.departmentName ?? "—";
                  const deptDesc = dept.Description ?? dept.description;
                  return (
                    <button
                      key={deptId ?? i}
                      onClick={() =>
                        setSelected(selected === deptId ? null : deptId)
                      }
...
function KickFromDepartmentModal({
  member,
  clubId,
  onClose,
}: {
  ...
  const { show } = useNotification();
  const { data: departments, isLoading } = useGetMemberJoinedDepartmentsQuery({
    clubId,
    memberId: member.clubMemberId,
  });
  ...
  const handleRemove = async () => {
    ...
    const deptName =
      confirmDept.Name ?? confirmDept.name ?? confirmDept.departmentName ?? "—";
    try {
      await removeMember({
        clubId,
        departmentId: deptId,
      ...
      show({
        type: "success",
        title: "Đã xóa",
        message: `${member.fullName} đã được xóa khỏi "${deptName}".`,
        duration: 3000,
      });
      setConfirmDept(null);
      ...
      show({
        type: "error",
        title: "Thất bại",
        message: err?.data?.message ?? "Không thể xóa khỏi phòng ban.",
        duration: 4000,
      });
    }
  };
      ...
      >
        ...
        >
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300 mb-6 leading-relaxed">
            ...
            ? Thành viên vẫn còn trong câu lạc bộ.
          </p>
          <div className="flex gap-3 justify-end">
            <button
              onClick={() => setConfirmDept(null)}
            ...
            <button
              ...
              disabled={isRemoving}
              className="px-4 py-2 rounded-xl bg-red-500 hover:bg-red-600 text-white text-sm font-bold flex items-center gap-2 shadow-lg shadow-red-500/20 disabled:opacity-50 transition-all cursor-pointer"
            >
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-4 max-h-72 overflow-y-auto">
          {isLoading ? (
            <div className="py-8 flex justify-center">
              <i className="fas fa-spinner fa-spin text-red-400 text-xl" />
            </div>
          ) : !departments || departments.length === 0 ? (
            <div className="py-10 text-center text-sm text-gray-400">
              <i className="fas fa-building text-3xl mb-2 opacity-30 block" />
            ...
            <div>
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
                Chọn phòng ban muốn xóa thành viên khỏi:
              </p>
              <div className="space-y-2">
                {departments.map((dept: any, i: number) => {
                  const deptId = dept.DepartmentId ?? dept.departmentId;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-semibold text-gray-900 dark:text-white truncate group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors">
                          {deptName}
                        </p>
                        {deptDesc && (
                          <p className="text-xs text-gray-500 dark:text-gray-400 truncate">
...
function KickFromClubModal({
  ...
  const handleRemove = async () => {
    try {
      ...
      show({
        type: "success",
        title: "Đã xóa khỏi CLB",
        message: `${member.fullName} đã bị xóa khỏi câu lạc bộ.`,
        duration: 3000,
      });
      onSuccess();
    } catch (err: any) {
      show({
        type: "error",
        title: "Thất bại",
        message: err?.data?.message ?? "Không thể xóa thành viên.",
        duration: 4000,
      });
    }
  };
    ...
    >
      ...
      >
        ...
        <p className="text-sm text-gray-600 dark:text-gray-300 mb-6 leading-relaxed">
          ...
          khỏi câu lạc bộ? Hành động này sẽ xóa thành viên khỏi tất cả phòng ban
...
export default function ClubMembersModule() {
  ...
  const [toggleMemberStatus, { isLoading: isTogglingStatus }] =
    useToggleMemberStatusMutation();
  const { show } = useNotification();
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      show({
        ...
        message:
          err?.data?.message ?? "Không thể cập nhật trạng thái thành viên.",
        duration: 4000,
      });
    }
  };
