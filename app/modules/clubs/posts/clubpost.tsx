...
function CreatePostModal({
    onClose, clubId, userId,
}: { onClose: () => void; clubId: number; userId: string }) {
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4">
            <div className="bg-white w-full sm:rounded-3xl shadow-2xl sm:max-w-2xl max-h-[95vh] overflow-y-auto">
                ...
                <div className="sticky top-0 bg-white/95 backdrop-blur border-b border-zinc-100 px-6 py-4 flex items-center justify-between z-10 rounded-t-3xl">
                    <div>
                        <h2 className="text-base font-bold text-zinc-900">Tạo bài viết mới</h2>
                        <p className="text-xs text-zinc-400 mt-0.5">Điền đầy đủ thông tin để đăng bài</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    ...
                    {/* Caption */}
                    <div className="space-y-1.5">
                        <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider">Mô tả ngắn</label>
                        <input
                            type="text" name="caption" value={form.caption} onChange={handleChange}
                    ...
                    {/* Content */}
                    <div className="space-y-1.5">
                        <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider">Nội dung</label>
                        <textarea
                            name="content" rows={5} value={form.content} onChange={handleChange}
                    ...
                    {/* Image Upload */}
                    <div className="space-y-1.5">
                        <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider">Ảnh minh họa</label>
                        ...
                        {imagePreview ? (
                            ...
                            >
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${dragOver ? 'bg-orange-100' : 'bg-zinc-100'}`}>
                                    <FileImage size={22} strokeWidth={1.5} />
                                </div>
                                <div className="text-center">
                                    <p className="text-xs font-semibold">Kéo thả hoặc nhấn để chọn ảnh</p>
                                    <p className="text-[10px] mt-0.5 opacity-60">JPG, PNG, WEBP · tối đa 10MB</p>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    {/* Status */}
                    <div className="space-y-1.5">
                        <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider">Trạng thái</label>
        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-gray-900">
            ...
            <main className={`pt-24 p-6 transition-all duration-300 min-h-screen ${isSidebarOpen ? 'md:ml-64' : 'ml-0'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                    <StatCard label="Tổng bài viết" value={clubPosts.length} icon={FileText} gradient="bg-gradient-to-br from-zinc-700 to-zinc-900" />
                    <StatCard label="Công khai" value={published} icon={CheckCircle2} gradient="bg-gradient-to-br from-green-500 to-emerald-600" />
                    <StatCard label="Đã ẩn" value={hidden} icon={EyeOff} gradient="bg-gradient-to-br from-red-400 to-rose-600" />
                </div>

                {/* ── Toolbar ── */}
                <div className="flex items-center justify-between mb-5">
                    <div>
                        <h2 className="text-base font-bold text-zinc-900 dark:text-white">Danh sách bài viết</h2>
                        <p className="text-xs text-zinc-400 mt-0.5">{clubPosts.length} bài viết trong hệ thống</p>
                    </div>
                ...
                {/* ── Content ── */}
                {isLoading ? (
                    <div className={`${viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4' : 'space-y-3'} animate-pulse`}>
                        {[1, 2, 3, 4].map(i => (
                            <div key={i} className={`bg-zinc-200 rounded-2xl ${viewMode === 'grid' ? 'h-64' : 'h-24'}`} />
                        ))}
                    </div>
                ) : clubPosts.length === 0 ? (
                    <div className="bg-white rounded-3xl shadow-sm border border-zinc-100 text-center py-20 px-6">
                        <div className="w-16 h-16 bg-orange-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
                            <TrendingUp size={28} className="text-orange-400" strokeWidth={1.5} />
                        </div>
                        <h3 className="font-bold text-zinc-900 mb-1">Chưa có bài viết nào</h3>
                        <p className="text-sm text-zinc-400 mb-5">Hãy đăng bài viết đầu tiên của câu lạc bộ</p>
                        <button onClick={() => setShowModal(true)}
                            ...
                            <div key={post.postId}
                                ...
                                <div className="relative h-40 bg-zinc-100 overflow-hidden">
                                    {post.imageUrl
                                        ...
                                        : <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-orange-50 to-orange-100"><FileImage size={32} className="text-orange-200" strokeWidth={1} /></div>
                                    }
                                    <div className="absolute top-2.5 right-2.5"><StatusBadge status={post.status} /></div>
                                </div>
                                {/* Body */}
                                <div className="p-4">
                                    <h3 className="font-bold text-zinc-900 text-sm line-clamp-2 leading-snug mb-1">{post.title}</h3>
                                    {post.caption && <p className="text-xs text-zinc-400 italic line-clamp-1 mb-3">— {post.caption}</p>}
                                    <div className="flex items-center gap-3 text-[11px] text-zinc-400">
                                        <span className="flex items-center gap-1"><User size={10} />{post.userName}</span>
                                        <span className="flex items-center gap-1"><Calendar size={10} />{new Date(post.postDate).toLocaleDateString('vi-VN')}</span>
                                    </div>
                                </div>
                                {/* Actions */}
                                <div className="px-4 pb-4 flex items-center gap-2 border-t border-zinc-50 pt-3">
                                    ...
                                    <button onClick={() => handleToggleStatus(post.postId, post.status || '')}
                                        ...
                                        {post.status === 'inactive' ? <Eye size={12} /> : <EyeOff size={12} />}
                            ...
                            <div key={post.postId}
                                ...
                                <div className="flex items-stretch">
                                    ...
                                    <div className="w-28 sm:w-36 shrink-0 bg-zinc-100 overflow-hidden">
                                        {post.imageUrl
                                            ? <img src={post.imageUrl} alt={post.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                                            : <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-orange-50 to-orange-100 min-h-[88px]"><FileImage size={22} className="text-orange-200" strokeWidth={1} /></div>
                                        }
                                    </div>
                                    ...
                                    <div className="flex-1 px-4 py-3.5 flex flex-col justify-between min-w-0">
                                        <div>
                                            <div className="flex items-start gap-2 mb-1">
                                                <h3 className="flex-1 font-bold text-zinc-900 text-sm line-clamp-1 leading-snug">{post.title}</h3>
                                                <StatusBadge status={post.status} />
                                            </div>
                                            {post.caption && (
                                                <p className="text-xs text-zinc-400 italic line-clamp-1 mb-1">— {post.caption}</p>
                                            )}
                                            {post.content && (
                                                <p className="text-xs text-zinc-500 line-clamp-1 leading-relaxed">{post.content}</p>
                                            )}
                                        </div>
                                    ...
                                    <div className="shrink-0 flex items-center gap-1 px-3 border-l border-zinc-50">
                                        ...
                                        <button onClick={() => handleToggleStatus(post.postId, post.status || '')}
                                            ...
                                            {post.status === 'inactive' ? <Eye size={15} /> : <EyeOff size={15} />}
                                        </button>
                                        <button onClick={() => handleDelete(post.postId)}
                                            title="Xóa"
