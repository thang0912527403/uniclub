...
import { useGetClubReportSummaryQuery, useGetClubReportAnalyticsQuery } from '~/cores/api';
...
export default function DashboardModule() {
  ...
  const { data: apiData, isLoading, isFetching, error } = useGetClubReportSummaryQuery(
    { clubId, year, month },
    { skip: !hasClubId },
  );
  const {
  ...
  const adminReport = apiData?.success === true ? apiData.data.adminReport : null;
  const analyticsData = analyticsApiData?.success === true ? analyticsApiData.data : null;
  const showLoading = isLoading || isFetching || isAnalyticsLoading || isAnalyticsFetching;
  ...
  const statCards = useMemo(() => {
    ...
    if (adminReport?.totalClubs != null) {
      cards.unshift({
        key: 'totalClubs',
        label: 'Total Clubs',
        value: adminReport.totalClubs,
        sub: 'Registered clubs',
        icon: 'fa-building',
      ...
      <main className={`pt-24 p-6 bg-gray-50 dark:bg-gray-900 transition-all duration-300 min-h-screen ${isSidebarOpen ? 'md:ml-64' : 'ml-0'}`}>
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Club performance metrics for the selected period</p>
          </div>
        ...
        {showLoading && <Loading message="Loading dashboard..." />}
        {!showLoading && hasClubId && (error || analyticsError) && (
          <Error title="Failed to load dashboard data." error={error ?? analyticsError} />
        )}

        {!showLoading && !error && !analyticsError && data && (
          <>
            {/* Hero banner */}
            <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 via-blue-700 to-purple-700 rounded-2xl shadow-lg p-6 mb-8 text-white">
              ...
              <div className="relative flex items-center justify-between flex-wrap gap-4">
                <div>
                  ...
                  <p className="text-white/60 text-sm mt-1">Real-time club statistics</p>
                </div>
                <div className="flex gap-3 flex-wrap">
                  {[
                    ...
                    <div key={item.label} className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl px-4 py-3 text-center min-w-[76px]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4 mb-8">
              {statCards.map(card => (
                <div key={card.key} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
              ...
              <div className="xl:col-span-2 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                <div className="flex items-start justify-between mb-5">
                  <div>
                    <h3 className="text-base font-bold text-gray-900 dark:text-white">Member Growth</h3>
                    <p className="text-xs text-gray-400 mt-0.5">New members per month – {analyticsData?.membershipGrowth.year ?? year}</p>
                  </div>
                  {analyticsData?.membershipGrowth.bestGrowthMonth && (
                    <span className="px-2.5 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-xs font-semibold rounded-full whitespace-nowrap">
                      Peak T{analyticsData.membershipGrowth.bestGrowthMonth} · {analyticsData.membershipGrowth.bestGrowthCount} new
                    </span>
                  )}
                </div>
                <div className="h-60">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 flex flex-col">
                <div className="mb-5">
                  <h3 className="text-base font-bold text-gray-900 dark:text-white">Event Participation</h3>
                  <p className="text-xs text-gray-400 mt-0.5">Check-in status this month</p>
                </div>
                  ...
                  <>
                    ...
                    <div className="mt-auto space-y-2">
                      {eventParticipationData.map((item, i) => {
                        const total = eventParticipationData.reduce((s, d) => s + d.value, 0);
