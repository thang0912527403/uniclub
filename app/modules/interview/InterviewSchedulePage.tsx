...
const InterviewSchedulePage: React.FC = () => {
  ...
  const { data: selectedInterview } = useGetInterviewByIdQuery(
    selectedInterviewId!,
  ...
  const statusCounts = useMemo(() => {
    ...
    allInterviews.forEach((iv) => {
      ...
      if (iv.status === "Completed") {
        ...
        const done =
          iv.assignments?.filter((a) => a.feedbackSubmittedAt).length || 0;
  ...
  const pipelineTabs: PipelineTab[] = useMemo(
    () => [
      ...
      },
      {
        key: "PendingFeedback",
        label: "Chờ đánh giá",
        count: statusCounts.PendingFeedback,
        color: "#f59e0b",
        icon: (
          <svg
            className="w-4 h-4"
        ...
        async onOk() {
          ...
          try {
            await updateStatus({
              ...
              dto: {
                status: "Cancelled",
                cancelReason: cancelReasonRef.current.trim(),
              },
            }).unwrap();
  ...
  const handleBulkCancel = () => {
    ...
    Modal.confirm({
      ...
      content: (
        <div>
          <p className="mb-2">
            Nhập lý do hủy cho tất cả ({selectedIds.size} lịch):
          </p>
          <Input.TextArea
            rows={3}
            placeholder="Lý do hủy..."
            onChange={(e) => {
              bulkCancelReasonRef.current = e.target.value;
            }}
          />
        </div>
      ),
        ...
        for (const id of selectedIds) {
          try {
            await updateStatus({
              ...
              dto: {
                status: "Cancelled",
                cancelReason: bulkCancelReasonRef.current.trim(),
              },
            }).unwrap();
            ok++;
          } catch {}
        ...
        {activeCampaignId ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            <p className="text-sm mt-1">
              Vui lòng tạo chiến dịch tuyển dụng trước.
            </p>
          </div>
        )}
      </main>
