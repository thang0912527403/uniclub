...
/** Stored as JSON in the description field for candidate to pick */
...
const CreateInterviewModal: React.FC<CreateInterviewModalProps> = ({
  ...
  const targetApps = isBulkMode ? applications : application ? [application] : [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // The first valid slot becomes scheduledAt, all slots are stored as JSON in description
    const firstSlot = validSlots[0];
    const scheduledAt = new Date(`${firstSlot.date}T${firstSlot.time}`).toISOString();

    // If multiple slots proposed, embed them as JSON so candidate can pick later
    let fullDescription = description.trim();
    if (validSlots.length > 1) {
      const proposedData: ProposedSlots = {
        proposedTimeSlots: validSlots.map(s => ({ date: s.date, time: s.time })),
      };
      // Append JSON block at end of description
      fullDescription = fullDescription
        ? `${fullDescription}\n\n<!--PROPOSED_SLOTS:${JSON.stringify(proposedData)}-->`
        : `<!--PROPOSED_SLOTS:${JSON.stringify(proposedData)}-->`;
    }
    ...
    try {
      ...
      if (created > 0) {
        const slotText = validSlots.length > 1 ? ` (${validSlots.length} khung giờ đề xuất)` : '';
        message.success(`Đã tạo ${created} lịch phỏng vấn${slotText} thành công!`);
      }
      if (failed > 0) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden animate-scaleIn max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto flex-1">
            ...
            {validSlots.length > 1 && (
              <div className="mt-2 px-3 py-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
                <p className="text-xs text-blue-600 dark:text-blue-400 flex items-center gap-1.5">
                  ...
                  Tạo 1 lịch PV{isBulkMode ? ` / ứng viên` : ''} — ứng viên sẽ chọn 1 trong {validSlots.length} khung giờ
                </p>
              </div>
            )}
          </div>
          ...
          {/* Duration */}
          <div>
            <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Thời lượng</label>
            <select
          ...
          {!isBulkMode && application && (
            <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 border border-gray-100 dark:border-gray-600">
              <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-2">Thông tin ứng viên</p>
          ...
          {isBulkMode && (
            <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 border border-gray-100 dark:border-gray-600">
              ...
              <div className="space-y-1.5 max-h-32 overflow-y-auto scrollbar-thin">
                {targetApps.map(app => (
                  <div key={app.applicationId} className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-300">
                    <span className="font-medium truncate">{app.userId}</span>
                    <span className="text-xs text-gray-400 dark:text-gray-500">#{app.applicationId}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={isSubmitting || !title.trim() || validSlots.length === 0}
