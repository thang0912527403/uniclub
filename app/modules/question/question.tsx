...
const QuestionPage: React.FC = () => {
  ...
  const actualFormId =
    campaignForms.length > 0 ? campaignForms[0].formId : idFromUrl;
  ...
  const {
    ...
    { formId: actualFormId },
    { skip: !actualFormId || isNaN(actualFormId) },
  );

  const [
    submitApplication,
    { isLoading: isSubmitting, error: submitError, isSuccess },
  ] = useSubmitApplicationMutation();

  // Check if user already applied to this form (avoid 404 by fetching all user apps and filtering)
  const { data: userApps = [], isLoading: checkingApp } =
    useGetApplicationsByUserQuery(
      { userId: currentUserId },
      { skip: !currentUserId },
    );
      ...
      <div className="min-h-screen bg-[#FDFCFB]">
        ...
        <main className="max-w-3xl mx-auto px-4 py-20 text-center">
          ...
          <p className="text-gray-500 mb-6">
            Đường dẫn không hợp lệ. Vui lòng chọn chiến dịch tuyển dụng trước.
          </p>
          <Link
            to="/recruitment-campaigns"
            className="px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white rounded-xl font-bold transition-all inline-block"
          >
        ...
        <main className="max-w-md mx-auto px-4 py-20 text-center">
          <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-10">
            ...
            <p className="text-gray-500 text-sm mb-6">
              Bạn cần đăng nhập để nộp đơn ứng tuyển. Vui lòng đăng nhập và thử
              lại.
            </p>
            <Link
            ...
            <Link
              to="/auth/register"
              className="block w-full py-3 border border-gray-200 text-gray-600 hover:text-gray-800 hover:bg-gray-50 rounded-xl font-medium transition-all text-sm"
            >
      ...
      <div className="min-h-screen bg-[#FDFCFB]">
        ...
        <main className="max-w-3xl mx-auto px-4 py-20 text-center">
          ...
          <p className="text-red-600 font-medium">
            Không thể tải biểu mẫu. Vui lòng thử lại.
          </p>
        </main>
      </div>
    );
        ...
        <main className="max-w-md mx-auto px-4 py-20 text-center">
          <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-10">
            ...
            <p className="text-xs text-gray-400 mb-6">
              Nộp lúc:{" "}
              {new Date(existingApp.submissionDate).toLocaleString("vi-VN")}
              {" • "}
              Trạng thái:{" "}
        ...
        <main className="max-w-3xl mx-auto px-4 py-16">
          ...
          <div className="mt-10 rounded-2xl border border-amber-200 bg-amber-50 p-6 text-amber-800">
            <p className="font-medium">
              ...
              Form chưa có câu hỏi.
            </p>
            <p className="mt-1 text-sm">
              Quản trị viên cần thêm câu hỏi vào form trước khi bạn có thể gửi
              đơn.
            </p>
          </div>
        </main>
          ...
          {questions.map((q) => (
            <QuestionCard
              ...
              inputRef={(el: HTMLElement | null) => {
                inputRefs.current[q.questionId] = el;
              }}
            />
          ))}
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-sm w-full mx-4 text-center">
            ...
            <p className="text-gray-500 text-sm mb-6">
              Bạn có chắc muốn nộp đơn ứng tuyển? Sau khi gửi, bạn sẽ không thể
              chỉnh sửa.
            </p>
