      ...
      <div className="py-12 px-4 max-w-3xl mx-auto">
        {isTestMode && (
          <div className="mb-6 rounded-xl border border-blue-200 bg-blue-50 dark:bg-blue-900/20 dark:border-blue-800 p-4 text-blue-800 dark:text-blue-200 text-sm">
            Đang xem ở chế độ test (chưa đăng nhập). Hiển thị đơn của tài khoản
            test.
          </div>
        )}
        ...
        {applications.length === 0 ? (
          ...
          <ul className="space-y-4">
            {applications.map((app) => {
              ...
              const color =
                statusColor[app.status] ?? "bg-gray-100 text-gray-800";
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-5 flex flex-wrap items-center justify-between gap-4">
                    <div>
                      <p className="text-lg font-bold text-indigo-600 dark:text-indigo-400 mb-1">
                        <FormNameDisplay formId={app.formId} />
                      </p>
                      <p className="text-gray-900 dark:text-white font-medium mt-0.5 text-sm">
                        Nộp lúc:{" "}
                        {new Date(app.submissionDate).toLocaleString("vi-VN")}
                      </p>
                      <span
                        className={`inline-block mt-2 px-2.5 py-1 rounded-full text-sm font-medium ${color}`}
                      >
                        {label}
                      </span>
                    </div>
...
function FormNameDisplay({ formId }: { formId: number }) {
  const { data: form, isLoading } = useGetFormByIdQuery({ id: formId });
  if (isLoading)
    return <span className="opacity-60 text-sm">Đang tải tên đơn...</span>;
  if (!form) return <span>Đơn ứng tuyển #{formId}</span>;
  return <span>{form.formTitle || form.formName}</span>;
}
