...
export function Sidebar({ 
  ...
  const navItems: NavItem[] = [
    ...
    { 
      ...
      icon: 'fa-building', 
      subItems: [
        { label: 'View Club Info', href: '/club/info' },
        { label: 'Manage Club Name', href: '/club/name' },
        { label: 'Club Settings', href: '/club/settings' },
        { label: 'Club Members', href: '/club/members' },
        { label: 'Club Activities', href: '/club/activities' },
      ]
    },
    { label: 'Manage Department', icon: 'fa-sitemap', href: '/department' },
    { label: 'Manage Members', icon: 'fa-users', href: '/members' },
    { label: 'Manage Events', icon: 'fa-calendar', href: '/events' },
    { label: 'Manage Funds', icon: 'fa-wallet', href: '/funds' },
  ];
    ...
    <aside className={`w-64 min-h-screen p-4 ${sidebarClass} transition-colors duration-300`}>
      ...
      <nav className="space-y-1">
        {navItems.map((item) => {
          ...
          const isSubItemActive = hasSubItems && item.subItems?.some(sub => sub.href === currentPath);
