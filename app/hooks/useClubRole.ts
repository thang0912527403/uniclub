...
export function useClubRole() {
  ...
  const { data: detailedInfo, isLoading: isDetailedLoading } =
  ...
  const globalRole =
    memberships.length > 0 ? memberships[0].globalRole : "User";

  // Admin hệ thống
  const isAdmin = isGlobalAdmin || globalRole === "Admin";

  const currentClub = memberships.find(
    (m) => Number(m.clubId) === Number(selectedClubId),
  );
  ...
  useEffect(() => {
    if (currentClub) {
      console.log(
        `[useClubRole] Club Selected: ${currentClub.clubName || currentClub.clubId}`,
      );
      ...
      console.warn(
        `[useClubRole] Club ID ${selectedClubId} found in Cookie but not in memberships!`,
      );
    }
  ...
  const can = (policyName: string, clubId?: number) => {
    ...
    const targetClub = memberships.find(
      (m) => Number(m.clubId) === targetClubId,
    );
  ...
  const isCurrentClubManager =
    currentClub?.clubRoles?.some((r) => Number(r.level) === 0) ?? false;
